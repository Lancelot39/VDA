...
class FreeLB(nn.Module):
    ...
    def adv_train(self, input_ids, attention_masks, token_type_ids, y, optim, scheduler, mlm_model):
        ...
        for i in range(self.step):
            ...
            kl_loss = 0.5 * self.kl_criterion(torch.log_softmax(logits, -1), torch.softmax(noise_logits, -1)) + \
                      0.5 * self.kl_criterion(torch.log_softmax(noise_logits, -1), torch.softmax(logits, -1))
